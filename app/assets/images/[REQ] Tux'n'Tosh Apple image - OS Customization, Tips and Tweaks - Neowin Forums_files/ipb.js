...
window.IPBoard = Class.create({
	...
	initialize: function()
	{
		...
		document.observe("dom:loaded", function(){
			...
			// Neowin - load staff bar from Ignition
			
			if ($('staff-bar'))
			{
				new Ajax.Request('/staffbar?ajax=true', {
					onSuccess: function(response) {
						var json = response.responseText.evalJSON();
						document.getElementById('staff-bar').innerHTML = json.data.html;
					}
				});
			}			
...
_global.prototype.global = {
	...
	* Toggle spammer
	*/
	toggleFlagSpammer: function( memberId, memberPosts, memberJoin, flagStatus )
	{
		if ( flagStatus == true )
		{
			if( confirm( "WARNING! this member joined on " +  memberJoin + " and has " + memberPosts + " posts, flag as spammer?" ) )
			{
				var tid	= 0;
				var fid	= 0;
				var sid	= 0;
